...
pub fn default_prefix_predicate(extension: &str) -> Box<dyn Fn(&str) -> bool + Send + Sync> {
    ...
    let prefixes: Vec<&'static str> = match extension {
        ...
        // TimescaleDB doesn't use a single prefix — nearly all
        // functions are user-facing SQL identifiers advertised via
        // the shim-interface `scalars`/`aggregates` inventory. The
        // prefix set below covers the small handful that share a
        // recognisable prefix (`hypertable_*`, `add_*`, `remove_*`,
        // `time_bucket*`, `interpolated_*`, ...) so a case still
        // maps to a top-level call even when the inventory is
